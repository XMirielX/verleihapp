...
async function loadProducts() {
  try {
    ...
    if (!res.ok) {
      ...
      console.error("Product API Fehler:", res.status, text);
      throw new Error(
        `Produkte konnten nicht geladen werden (${res.status})`
      );
    }
...
async function loadDistributions() {
  try {
    ...
    if (!res.ok) {
      const text = await res.text();
      console.error("Distribution API Fehler:", res.status, text);
      throw new Error(
        `Distributionen konnten nicht geladen werden (${res.status})`
      );
    }

    distributions = await res.json();

    console.log("Geladene Distributionen:", distributions);
    console.log("Anzahl Distributionen:", distributions.length);
...
function renderProductSelect(currentProductId = null) {
  ...
  products
    ...
    .sort((a, b) =>
      (a.bez || "").localeCompare(b.bez || "", "de", { numeric: true }),
    )
    .forEach((product) => {
